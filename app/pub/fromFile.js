                ...
                controller: function($scope) {
                    ...
                    function openTemplateProcessModal() {
                        ...
                        modalInstance.result.then(function() {
                            if ($scope.templateOptions.add) {
                                ...
                                console.log('CachedTemplateService')
                    ...
                    function createUpdateSingleResource() {
                        ...
                        if (!_.isEmpty($scope.createResources)) {
                            ...
                            console.log('APIService.kindToResource(resource.kind)', APIService.kindToResource(resource.kind))
                            DataService.create(APIService.kindToResource(resource.kind), null, resource, { namespace: $scope.input.selectedProject.metadata.name }).then(
                                ...
                                function() {
                                    if (!$scope.isDialog) {
                                        ...
                                        console.log("NotificationsService ", "success", _.capitalize(kind), " ", resource.metadata.name, " was successfully created.")
                                            // NotificationsService.addNotification({
                                            //     type: "success",
                                ...
                                function(result) {
                                    console.log("NotificationsService ", "from-file-error", "Unable to create the ", humanizeKind(resource.kind), " '", resource.metadata.name + "'.")
                                        // NotificationsService.addNotification({
                                        //     id: "from-file-error",
                                        //     type: "error",
                            ...
                            DataService.update(APIService.kindToResource(resource.kind), resource.metadata.name, resource, { namespace: $scope.input.selectedProject.metadata.name }).then(
                                ...
                                function() {
                                    if (!$scope.isDialog) {
                                        var kind = humanizeKind(resource.kind);
                                        console.log("NotificationsService ", "success", _.capitalize(kind), " ", resource.metadata.name, " was successfully updated.")
                                            // NotificationsService.addNotification({
                                            //     type: "success",
                                ...
                                function(result) {
                                    console.log("NotificationsService ", "from-file-error", "error", "Unable to update the " + humanizeKind(resource.kind) + " '" + resource.metadata.name + "'.")
                                        // NotificationsService.addNotification({
                                        //     id: "from-file-error",
                                        //     type: "error",
