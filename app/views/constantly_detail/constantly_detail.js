...
angular.module('console.constantly_detail', [
    ...
    .controller('constDetailCtrl', ['Toast','Tip','delorders','orders','Confirm','delvolume','volume','DeploymentConfig','persistent','$stateParams','$state', '$http', '$scope', '$rootScope',
        function(Toast,Tip,delorders,orders,Confirm,delvolume,volume,DeploymentConfig,persistent,$stateParams,$state, $http, $scope, $rootScope){
                        ...
                        orders.query({region:$rootScope.region,resource_name:$stateParams.name}, function (data) {
                            ...
                            if (data.length>0&&data[0].order.id) {
                                delorders.delete({id:data[0].order.id,action:"cancel",namespace:$rootScope.namespace}, function (data) {
                                    ...
                                    console.log('err',err);
                                    Tip.open('提示', '删除失败', '知道了', true).then(function () {
                                        console.log(err)
                                    })
                                })
