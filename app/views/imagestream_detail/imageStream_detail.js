...
angular.module('console.imagestream_detail', [
    {
        files: [
            'components/searchbar/searchbar.js',
            'views/imageStream_detail/imageStream_detail.css'
        ]
    }
])
    .controller('imageStreamDetailCtrl', ['Confirm', 'ModalPullImage', '$state', 'ImageStream', '$http', 'platformone', 'platformlist', '$location', '$rootScope', '$scope', '$log', 'ImageStreamTag', 'ImageStreamImage',
        function (Confirm, ModalPullImage, $state, ImageStream, $http, platformone, platformlist, $location, $rootScope, $scope, $log, ImageStreamTag, ImageStreamImage) {
            ...
            ImageStream.get({
                ...
                $scope.date = data;
                $scope.itemsCurIdx = data.status.tags[0].items.length - 1;
            ...
            $scope.editYaml = function () {
                $state.go('console.edit_yaml_file',{namespace: $rootScope.namespace,name: $scope.date.metadata.name, kind:$scope.date.kind});
            };
        }]);